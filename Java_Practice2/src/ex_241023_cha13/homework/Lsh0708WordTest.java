...
public class Lsh0708WordTest extends JFrame {
...
//				}
//			});
			addClickListener(wordLabel,word,meaning);
			// Map 메모리에 저장하는 로직.
			// 4
	...
	public void showRandomWord() {
		...
		words = new ArrayList<String>(wordLabelMap.keySet());
	...
	public void saveWord() {
		...
		int c;
		try {
			fout = new FileWriter("c:\\Temp\\wordTest.txt");
	...
	public void loadWord() {
		String csvFile = "c:\\Temp\\Lsh0708wordTestCSV.csv";
		try {
			String csvKey;
			String csvValue;
			...
			while ((line = br.readLine()) != null) {
				String[] csvStr = line.split(",");
				if (csvStr.length == 2) {
					csvKey = csvStr[0].trim();
					csvValue = csvStr[1].trim();
					
					wordMeaningLabelMap.put(csvKey, csvValue);
					JLabel scvLabel = new JLabel(csvKey);
					scvLabel.setCursor(new Cursor(Cursor.HAND_CURSOR));
					wordLabelMap.put(csvKey, scvLabel);
					
					addClickListener(scvLabel,csvKey,csvValue);
					wordPanel.add(scvLabel);
					// 레이아웃 업데이트
					wordPanel.revalidate();
					// 패널 그림을 다시 그리기.
					wordPanel.repaint();
